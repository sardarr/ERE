...
for xfl in xfiles:
    ...
    myarray=Getting_Authors('input_cb/'+str(xfl[10:-13])+'.cmp.txt.xml') #adding the authors of each text files to the list My array
   ...
   # myadd='data_orig/BT02162016/'+str(xfl[14:-13])+'.cmp.txt.xml'
    ...
    with open ('input_src/'+str(xfl[10:-13])+'.cmp.txt','r') as input_data:#
              ...
              #      temp_ere=age.split(" ")
                ...
                try:
                        ...
                        #ind here should be change to be accurate
                            ...
                            if (containsAny(filestring[ind-5:ind]) and phrasal_entity_flag==True) and 'NA' not in containsAny(filestring[ind-5:ind]) :
                                    ...
                                    source=m_id_return(str(xfl[10:-13]),int(events.split()[-2]))
                                    source_id_n_offset=em_poseter_table[source]
                                    source_id=source_id_n_offset.split(',')[0]
                                    offset=source_id_n_offset.split(',')[1]
                                    print (events.split()[-1],item[0],events.split()[-2],events.split()[2],containsAny(filestring[ind-5:ind]),source_id,offset,str(len(source_id)),str(source))#Sample out put <NA> coming em-791 | admirable m-192 1222 9

                                    if "relm" in events.split()[-1]:
                                        if rel_flag==False:
                                            rltions = ET.SubElement(Belief_ano, 'relations')
                                            rel_flag=True
                                        creat_xml_relat(events.split()[-1],item[0],events.split()[-2],events.split()[2],containsAny(filestring[ind-5:ind]),source_id,offset,str(len(source_id)),str(source))
                                    if "em" in events.split()[-1]:
                                        if ev_flag==False:
                                            evnts = ET.SubElement(Belief_ano, 'events')
                                            ev_flag=True
                                        creat_xml_event(events.split()[-1],item[0],events.split()[-2],events.split()[2],containsAny(filestring[ind-5:ind]),source_id,offset,str(len(source_id)),str(source))
                except:
                    pass


    if check:
        output_file = open( 'pred_out/'+str(xfl[10:-13])+'.best.xml', 'w' )

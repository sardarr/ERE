...
def list_ind(of_array,ind):
    ...
    for i in range(len(of_array)):
        if abs(int(of_array[i])-int(ind))<tmp:
            tmp=abs(int(of_array[i])-int(ind))
            fin_ind=i
    return fin_ind
                ...
                for atrib in item:
                    for ent_men in atrib:
                        ...
                        if ent_men.text in myarray:# ent_men should change to actual poster
                            poester_hash[str(atrib.get('offset'))]=ent_men.text###***
                            poster_offset.append(int(atrib.get('offset')))###***
                            postrs_id[ent_men.text]=item.get('id')
        ...
        if child.tag == "relations"or"hopper":
            for item in child.iter('relation_mention'):
                for elm in item:
                    ...
                    if len(temp_elemrecord)>3:#this is bigger than three because size of temp_elm if has the entity id will be larger than 3 and this is what we are looking for
...
xfiles=glob.glob(path_ere)
for xfl in xfiles:
    Author_ere.clear()#This is a hashtable which stroes poster and their id for each input
    print xfl #printing the xml file address that is being processed
    myarray=Getting_Authors('belief_output/'+str(xfl[8:-13])+'.cmp.txt.xml') #adding the authors of each text files to the list My array
   ...
   # myadd='data_orig/BT02162016/'+str(xfl[14:-13])+'.cmp.txt.xml'
    with open('belief_output/'+str(xfl[8:-13])+'.cmp.txt.xml','r') as mytagged: #reading the CB output for the old data xfl [14:13] works
        ...
        splitar=filestring.split("</post>")
    with open ('input_txt/'+str(xfl[8:-13])+'.cmp.txt','r') as input_data:#
              ...
              #      temp_ere=age.split(" ")
                ...
                try:
                        ...
                        #ind here should be change to be accurate
                            ...
                            name=poester_hash[str(find_poster(int(events.split()[-2])))]
                            poster_offset.remove(int(events.split()[-2]))
                            if (containsAny(filestring[ind-4:ind]) and phrasal_entity_flag==True):
                                    if len(event_arr)>1:
                                        phrasal_entity_flag=False
                                    check=True
                                    #Printing all the taggs in each file
                                    print (filestring[ind-4:ind]),item[0],events.split()[-1],"|",name,Author_ere[name].split(" ")[0],Author_ere[name].split(" ")[2],Author_ere[name].split(" ")[3]#Sample out put <NA> coming em-791 | admirable m-192 1222 9
                                    if "relm" in events.split()[-1]:
                                        if rel_flag==False:
                                            rltions = ET.SubElement(Belief_ano, 'relations')
                                            rel_flag=True
                                        creat_xml_relat(events.split()[-1],item[0],events.split()[3],events.split()[2],filestring[ind-4:ind][1:-1],name,Author_ere[name].split(" ")[2],Author_ere[name].split(" ")[3],Author_ere[name].split(" ")[1])
